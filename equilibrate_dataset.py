...
def _build_arg_parser():
    ...
    parser.add_argument('dataset1', type=str, help='Path to the first dataset (an HDF5 file)')
    parser.add_argument('dataset2', type=str, help='Path to the second dataset which is to be modified (an HDF5 file)')
    parser.add_argument('output', type=str, help='Path to the output dataset (an HDF5 file)')
    parser.add_argument('--nb_splits', type=int, default=1, help='Number of random splits that match the input data.')
    return parser
...
def main():
    ...
    with h5py.File(args.dataset1, 'r') as f1:
        train_data1 = np.asarray(f1['train']['data'])
        train_scores1 = np.asarray(f1['train']['scores'])
        test_data1 = np.asarray(f1['test']['data'])
    ...
    train_indices_to_keep_splits = match_balance(train_data1, train_scores1, train_data2, train_scores2, nb_splits=nb_splits)
    print('Matching balance of the test set. Generating {} splits...'.format(nb_splits))
    test_indices_to_keep_splits = match_balance(test_data1, test_scores1, test_data2, test_scores2, nb_splits=nb_splits)
    ...
    for i, (train_indices_to_keep, test_indices_to_keep) in enumerate(zip(train_indices_to_keep_splits, test_indices_to_keep_splits)):
        ...
        split_filepath = os.path.join(out_dirpath, "split_{}_{}".format(i, os.path.basename(args.output)))
        ...
        new_train_data, new_train_scores = train_data2[train_indices_to_keep], train_scores2[train_indices_to_keep]
...
def match_balance(data1: np.ndarray, scores1: np.ndarray, data2: np.ndarray, scores2: np.ndarray, nb_splits: int = 1):
    ...
    print(f"Will resample the class \"{class_to_resample}\" from {initial_num_samples} to {num_samples_to_resample} num samples.")

    # Get the indices of the class to resample
    score_of_class_to_resample = 1 if class_to_resample == 'pos' else 0
    indices_of_other_class = np.arange(len(scores2))[scores2 != score_of_class_to_resample]
    indices_to_resample = np.arange(len_dataset2)[scores2 == score_of_class_to_resample]

    # Resample the indices
    resampled_indices_splits = []
    for _ in range(nb_splits):
        resampled_indices = np.random.choice(indices_to_resample, num_samples_to_resample, replace=False)
    ...
    indices_to_keep_splits = [np.concatenate([indices_of_other_class, ri]) for ri in resampled_indices_splits]

    # Evaluate the new balance of the dataset
    perc_pos_new = np.mean(scores2[indices_to_keep_splits[0]])
    perc_neg_new = 1 - perc_pos_new
    print('New balance of the set: {:.2f}% pos, {:.2f}% neg'.format(perc_pos_new * 100, perc_neg_new * 100))
    print('Difference in unbalanceness: {:.2f}'.format((perc_pos_new - perc_neg_new) - unbalanceness_1))
    assert np.abs((perc_pos_new - perc_neg_new) - unbalanceness_1) < unbalanceness_threshold, "Balance between the two datasets is not the same."
